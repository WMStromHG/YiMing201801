var nai_canvas, nai_stage, an_nai, fnStartAnimation;
function nai_init() {
    nai_canvas = document.getElementById("nai_canvas");
    an_images = an_images||{};
    var loader = new an_createjs.LoadQueue(false);
    loader.addEventListener("fileload", handleFileLoad);
    loader.addEventListener("complete", handleComplete);
    loader.loadManifest(an_lib.properties.manifest);
}
function handleFileLoad(evt) {
    if (evt.item.type == "image") { an_images[evt.item.id] = evt.result; }
}
function handleComplete(evt) {
    //This function is always called, irrespective of the content. You can use the variable "stage" after it is created in token create_stage.
    var queue = evt.target;
    var ssMetadata = an_lib.ssMetadata;
    for(i=0; i<ssMetadata.length; i++) {
        ss[ssMetadata[i].name] = new createjs.SpriteSheet( {"images": [queue.getResult(ssMetadata[i].name)], "frames": ssMetadata[i].frames} )
    }
    an_nai = new an_lib.yiming201801t3();
    an_nai.stop();

    nai_stage = new an_createjs.Stage(nai_canvas);
    nai_stage.addChild(an_nai);
    //Registers the "tick" event listener.
    fnStartAnimation = function() {
        an_createjs.Ticker.setFPS(an_lib.properties.fps);
        an_createjs.Ticker.addEventListener("tick", nai_stage);
    }
    fnStartAnimation();
}
